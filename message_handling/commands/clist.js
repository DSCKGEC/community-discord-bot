    ...
    request('https://clist.by/', (error, response, html) => {
        if (error) {
            ...
            message.channel.send(
                `The bot got unexpected responses from the site`
            );
            return;
        }
        ...
        $(`.row.contest.${type}`).each((i, el) => {
            ...
            let contestURL = $(el)
                .find('.event > .contest_title > a[title]')
                .attr('href');

            let contestHost = $(el)
                .find('.event > .resource > a > small')
                .text();

            //the timings better extracted from the data-ace of calenders. They are well formatted
            let contestTimings = JSON.parse(
                $(el).find('.event > a.data-ace').attr('data-ace')
            ).time;
