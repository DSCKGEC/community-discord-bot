...
const genericHelpMessage = new Discord.MessageEmbed()
    ...
    .addFields(
        { name: '!ping', value: 'Check whether our bot is alive or not!' },
        {
            name: '!welcome @user',
            value:
                'Send a welcome message for mentioned user on the default channel they landed on.',
        },
        {
            name: '!boost @user',
            value: `This command is for appreciating members for their valuable contributions to the community by rewarding them with a special role ⚡`,
        },
        {
            name: '!submit-idea',
            value:
                'If you have any project or event ideas, drop them in using this command!',
        },
        {
            name: '!scrape',
            value:
                'Currently scrapes latest news and events from DSC official website',
        },
        {
            name: '!clist-up',
            value:
                'Get a list of upcomng CP contests, passing an optional parameter for the limit.',
        },
        {
            name: '!clist-live',
            value:
                'Get a list of already going contests, passing an optional parameter for the limit.',
        },
        {
            name: '!report',
            value:
                'Have any issue or complaint? Use this command to send a secret report to the server admins',
        },
        {
            name: '!add-event',
            value:
                'If you are a server Admin, you can announce about latest event!',
        },
        {
            name: 'New user joins',
            value:
                'DM the user asking for IRL name and college. and setting their nicknames and roles accordingly',
        },
        {
            name: 'Adding domain badge to any user',
            value:
    ...
    if (args.length === 0) {
        ...
        switch (commandEnquired) {
            ...
            case 'help':
                ...
                message.channel.send(
                    randomJokes[Math.floor(Math.random() * 3)]
                );
